...
var Script = function Script(from) {
  ...
  } else if (_instanceof(from, Address)) {
    return Script.fromAddress(from);
  } else if (_instanceof(from, Script)) {
...
Script.prototype.equals = function (script) {
  $.checkState(_instanceof(script, Script), 'Must provide another script');
...
Script.prototype._addByType = function (obj, prepend) {
  if (typeof obj === 'string') {
    ...
    this._addOpcode(obj, prepend);
  } else if (_instanceof(obj, Opcode)) {
    this._addOpcode(obj, prepend);
  } else if (BufferUtil.isBuffer(obj)) {
    this._addBuffer(obj, prepend);
  } else if (_instanceof(obj, Script)) {
...
Script.prototype._addOpcode = function (opcode, prepend) {
  ...
  if (typeof opcode === 'number') {
    op = opcode;
  } else if (_instanceof(opcode, Opcode)) {
    op = opcode.toNumber();
  } else {
...
Script.buildPublicKeyHashOut = function (to) {
  ...
  $.checkArgument(_instanceof(to, PublicKey) || _instanceof(to, Address) || _.isString(to));

  if (_instanceof(to, PublicKey)) {
    to = to.toAddress();
...
Script.buildPublicKeyOut = function (pubkey) {
  $.checkArgument(_instanceof(pubkey, PublicKey));
  var s = new Script();
...
Script.buildScriptHashOut = function (script) {
  $.checkArgument(_instanceof(script, Script) || _instanceof(script, Address) && script.isPayToScriptHash());
  var s = new Script();
  s.add(Opcode.OP_HASH160).add(_instanceof(script, Address) ? script.hashBuffer : Hash.sha256ripemd160(script.toBuffer())).add(Opcode.OP_EQUAL);
...
Script.buildPublicKeyIn = function (signature, sigtype) {
  $.checkArgument(_instanceof(signature, Signature) || BufferUtil.isBuffer(signature));
