...
function Transaction(serialized) {
  if (!_instanceof(this, Transaction)) {
  ...
  if (serialized) {
    if (_instanceof(serialized, Transaction)) {
...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  $.checkArgument(_.isObject(arg) || _instanceof(arg, Transaction));
  var self = this;
  var transaction;

  if (_instanceof(arg, Transaction)) {
...
Transaction.prototype.addInput = function (input, outputScript, satoshis) {
  ...
  if (!input.output && outputScript && !_.isUndefined(satoshis)) {
    outputScript = _instanceof(outputScript, Script) ? outputScript : new Script(outputScript);
...
Transaction.prototype.verifySignature = function (sig, pubkey, nin, subscript, sigversion, satoshis) {
  ...
  if (sigversion === 1) {
    ...
    var verified = SighashWitness.verify(this, sig, pubkey, nin, scriptCodeWriter.toBuffer(), satoshisBuffer);
    return verified;
  }
